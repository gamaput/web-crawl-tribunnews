...
class TribunJatengSpider(scrapy.Spider):
    name = "kupang_tribun"
    allowed_domains = [ 'kupang.tribunnews.com' ]
    page_url = 'https://kupang.tribunnews.com/index-news?date='
    start_urls = [
        'https://kupang.tribunnews.com/index-news?date=2022-12-15&page=1'
    ...
    def parse(self, response):
        ...
        if 0 < n_news:
            ...
            if None == next_url:
                ...
                self.logger.info('\n >> Previous date %s\n', prev_date_str)
                next_url = self.page_url + prev_date_str + "&page=1"
                self.logger.info('\n >> PROCESSING in scrapy request %s\n', next_url)
    ...
    def parse_news_page(self, response):
        ...
        # content = response.xpath('//div[@class="side-article txt-article"]\
        #         //p[not(@class="baca")]//text()').getall()
